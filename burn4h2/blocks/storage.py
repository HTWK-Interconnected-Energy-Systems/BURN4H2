...
class BatteryStorage:
    ...
    This class creates a battery storage system that can store
    electrical energy and provide it back to the system when needed.
    """
    ...
    def battery_storage_block_rule(self, block):
        ...
        # Add optional constraint for cyclic behavior when specified in kwargs
        if 'cyclic_behaviour' in self.kwargs:             
            ...
            def cyclic_switch_rule(_block, i):
                """Rule for determining cyclic switching behavior."""
            ...
            def cyclic_behaviour_rule(_block, i):
                """Rule for limiting switching operations within a cycle."""
                if i % kwarg_value == 0:
            ...
            block.cyclic_behavior_constraint = Constraint(
                t,
...
class HydrogenStorage:
    ...
    This class creates a hydrogen storage system that can store
    hydrogen and provide it back to the system when needed.
    """
    ...
    def hydrogen_storage_block_rule(self, block):
        ...
        block.actual_hydrogen_content_constraint = Constraint(
            t,
...
class HeatStorage:
    ...
    This class creates a heat storage system that can store
    thermal energy and provide it back to the system when needed.
    """
...
class GeoHeatStorage:
    ...
    This class creates a simple geothermal heat storage system that can 
    store thermal energy in the ground and extract it when needed.
    """
    ...
    def geo_heat_storage_block_rule(self, block):
        """Rule for creating a geothermal heat storage block with default components
        and constraints."""
...
class StratifiedHeatStorage:
    """Class for constructing a stratified heat storage system.
    
    This class creates a representation of a double heat storage system with
    district heating and local heating components separated into two temperature zones.
    """
    ...
    def stratified_storage_block_rule(self, block):
        """Rule for creating a stratified heat storage block with district and local heating components.

        This block includes two storage layers: 
        Z1: First Layer Storage (district heating)
        Z2: Second Layer Storage (local heating)
        """
        ...
        # Declare parameters
        block.delta_t = Param(initialize=1)  # Time step [h]
        ...
        block.initial_fill_percentage = Param(initialize=0)  # Initial fill level, 1 = 100% energy content
        block.initial_volume_share_Z1 = Param(initialize=0.5)  # Share of district heating storage volume
        ...
        # Initial capacities
        block.init_capacity_Z1 = Param(
            initialize=calc_max_capacity_Z1() * block.initial_fill_percentage
        )  # Initial capacity of Z1 storage
        block.init_capacity_Z2 = Param(
            initialize=calc_max_capacity_Z2() * block.initial_fill_percentage
        )  # Initial capacity of Z2 storage
        ...
        block.Q_dot_Z1_FW = Var(t, domain=NonNegativeReals)  # Heat from Z1 to district heating [MW]
        block.Q_dot_Z1_NW = Var(t, domain=NonNegativeReals)  # Heat from Z1 to local heating [MW]
        block.Q_dot_Z2_NW = Var(t, domain=NonNegativeReals)  # Heat from Z2 to local heating [MW]
        ...
        # Heat loss variables
        block.Q_loss_Z1_actual = Var(t, domain=NonNegativeReals)  # Heat loss from Z1 [MW]
        block.Q_loss_Z2_actual = Var(t, domain=NonNegativeReals)  # Heat loss from Z2 [MW]

        # Declare ports
        block.st_heat_in = Port()
        block.st_heat_in.add(
            block.Q_dot_ST, 
            'st_heat', 
            Port.Extensive, 
            include_splitfrac=False
        ...
        def max_fw_discharge_rule(_block, i):
            """Rule for the maximum district heating discharge rate."""
            return _block.Q_dot_Z1_FW[i] <= self.data.loc['max', 'heat']
        
        def max_nw_discharge_rule(_block, i):
            """Rule for the maximum local heating discharge rate."""
        ...
        # U(t) = U(t−1) + [Q_in(t) - Q_out(t)] ⋅ Δt
        
        def Z1_storage_balance_rule(_block, i):
            """Rule for the energy balance of zone 1 (district heating) storage."""
            if i == 1:
        ...
        def Z2_storage_balance_rule(_block, i):
            """Rule for the energy balance of zone 2 (local heating) storage."""
            if i == 1:
        ...
        def min_Z1_capacity_rule(_block, i):
            """Rule for the minimum energy content of zone 1 storage."""
            return _block.U_Z1[i] >= 0
        
        def min_Z2_capacity_rule(_block, i):
            """Rule for the minimum energy content of zone 2 storage."""
            return _block.U_Z2[i] >= 0
        
        def physical_volume_constraint_rule(_block, i):
            """Rule for the physical volume constraint of the stratified storage."""
        ...
        # Add optional seasonal discharge restriction if specified in kwargs
        if self.kwargs.get('seasonal_discharge_restriction'):
            # Define time periods for seasonal restrictions
            block.fw_discharge_restricted_periods_dummy = Set(
                initialize=list(range(1, 11)) + list(range(12, 14))
            )  # Dummy periods for testing
            block.fw_discharge_restricted_periods = Set(
                initialize=list(range(1, 2879)) + list(range(7296, 8761))
            )  # Regular year restriction: Nov 1 to Apr 30
            block.fw_discharge_restricted_periods_leap = Set(
                initialize=list(range(1, 2903)) + list(range(7320, 8785))
            )  # Leap year restriction: Nov 1 to Apr 30

            def storage_fw_discharge_rule(_block, i):
                """Rule for seasonal discharge restrictions."""
